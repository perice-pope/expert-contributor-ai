...
# BUG (intentional): region is set in credentials but not in config file.
# AWS CLI requires region in config file for proper profile isolation.
# The region should be set in BOTH credentials (via aws configure set) AND config (via write_ini_value.py).
