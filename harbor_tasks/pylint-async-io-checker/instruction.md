...
2. **Detect blocking I/O in async functions**: The plugin must identify blocking I/O operations inside functions decorated with `@asyncio.coroutine`, `async def`, or methods of classes that inherit from async context managers. Blocking I/O includes:
...
- **Use Python 3.11+** with standard library `ast` module for parsing
