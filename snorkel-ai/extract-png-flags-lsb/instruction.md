...
2. **Extract images**: Save each carved PNG image to `/app/images/` with descriptive filenames (e.g., `image_0.png`, `image_1.png`, etc.). The dump contains decoys, so you should expect at least five carved PNGs.
3. **Extract LSB-encoded flags**: For each PNG image, extract ASCII text flags hidden in the least significant bits of RGB pixel data. Flags are encoded sequentially across pixels (R, G, B channels) until a null terminator or end of image data. Note: Not all carved PNG images will contain valid flags; some may contain noise or decoy data. You must identify and extract the valid flags corresponding to the expected set; some extracted data may be noise or decoys that should be filtered out. Valid flags match the pattern `FLAG{...}` with only letters, numbers, and underscores inside the braces.
4. **Document findings**: Write all recovered flags along with their byte offsets in the memory dump to `/app/flags.txt`. Use the byte offset of the PNG header signature (`89 50 4E 47 0D 0A 1A 0A`) that corresponds to the image containing the flag. Format: one flag per line with offset in hexadecimal (e.g., `0x1234: FLAG{example_flag_here}`). Include only valid flags that correspond to the expected forensic findings.
