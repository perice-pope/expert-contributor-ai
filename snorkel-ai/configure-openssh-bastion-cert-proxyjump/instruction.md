...
3. Update the client config (`/app/client/ssh_config`) to ProxyJump through the bastion, use the provided client key + certificate, keep `StrictHostKeyChecking yes`, and set `UserKnownHostsFile` to `/app/client/known_hosts` (this ensures only that file is used for host verification). The SSH client configuration must explicitly set CertificateFile to the provided client certificate.
