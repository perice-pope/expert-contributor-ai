...
2. Configure bastion sshd at `/app/bastion/sshd_config` (port 2222) to accept only the user CA for `appuser`, disable password and keyboard-interactive auth, and serve its host certificate. Include `AuthorizedKeysFile none` to enforce CA-only auth.
3. Configure internal sshd at `/app/internal/sshd_config` (port 2223) to use its host certificate, trust the same user CA via `AuthorizedPrincipalsFile` containing `appuser`, disable password and keyboard-interactive auth, and include `AuthorizedKeysFile none`.
4. Update the client config `/app/client/ssh_config` to use ProxyJump through the bastion, set `ControlMaster auto`, set `ControlPersist yes` (or `ControlPersist 5m`), keep a valid `ControlPath` (e.g., `/tmp/ssh-%r@%h:%p`) for multiplexing, set `StrictHostKeyChecking yes`, use the provided key + certificate, and only `/app/client/known_hosts` for host trust.
