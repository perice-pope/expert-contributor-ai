...
def test_mysql_lock_mechanism():
    """Test that backup script properly handles MySQL locks."""
    log_file = Path("/var/log/mysql_backup.log")
    if log_file.exists():
        log_content = log_file.read_text()
        ...
        has_lock_info = (
            ...
            "UNLOCK TABLES" in log_content
        )
        # This is a soft check - if logs mention locks, that's good
        # We don't fail if not found since the main test is that backup works
